...
-- base_amount range queries: user_id + base_amount + created_at
-- Optimizes queries with base_amount range filters
CREATE INDEX IF NOT EXISTS idx_transactions_base_amount_range 
ON public.transactions(user_id, base_amount, created_at DESC);
...
-- User + type + base_amount (for type-specific base_amount analytics)
CREATE INDEX IF NOT EXISTS idx_transactions_user_type_base_amount 
ON public.transactions(user_id, type, base_amount);

-- User + status + base_amount (for status-specific base_amount analytics)
CREATE INDEX IF NOT EXISTS idx_transactions_user_status_base_amount 
ON public.transactions(user_id, status, base_amount);
