...
export default function EnhancedCheckoutPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#1a2332] to-[#2a3441] flex items-center justify-center px-4">
      <div className="w-full max-w-[850px] flex rounded-xl overflow-hidden shadow-2xl">
        ...
        <div className="flex-1 bg-[#3a4a5c] p-8 relative">
          ...
          <div className="flex flex-col h-full">
            ...
            {/* Dynamic Payment Method Icons */}
            <div className="flex gap-3 mb-8 flex-wrap">
              {getPaymentIcons().map((icon, index) => (
                <div key={index} className="w-16 h-12 bg-white rounded flex items-center justify-center p-1 shadow-sm">
                  <Image
                    src={icon.src}
                    alt={icon.alt}
                    width={48}
            ...
            <div className="mb-8">
              ...
              <input
                ...
                className="w-full px-4 py-3 bg-[#2a3441] border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:border-[#4CAF50] focus:outline-none transition-colors"
              />
            </div>

            {/* Pay Button */}
            <button className="w-full bg-[#8BC34A] hover:bg-[#7CB342] text-white font-bold py-4 rounded-lg transition-colors duration-200 mb-8 shadow-lg">
              Pay ETB 1.00
            </button>
